...
class QuipProcessor {
    ...
    _renderDocumentHtml(html, title, path) {
        ...
        const documentRenderOptions = {
            ...
            script_path: '../'.repeat(pathDeepness) + 'document.js'
        };
    ...
    async _getThreadMessagesHtml(quipThread, path) {
        ...
        for (const message of messages) {
            const annotationId = message && message.annotation && message.annotation.id ? message.annotation.id : '_';
            if (!(annotationId in groupedMessages)) {
                groupedMessages[annotationId] = [];
            }
            groupedMessages[annotationId].push(message);
        }


        for (const annotationId in groupedMessages) {
            const cssClass = 'comments' + (annotationId == '_' ? ' visible' : '');
            html += `<div id="comments-${annotationId}" class="${cssClass}">`;

            for(const message of groupedMessages[annotationId]) {
