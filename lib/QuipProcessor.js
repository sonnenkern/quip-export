...
class QuipProcessor {
    ...
    async _processDocumentThread(quipThread, path) {
        ...
        regexp = new RegExp(`href=\\"(${this.quipUser.url}/(.+))\\"`, 'gim');
        const matchesReference = this._getMatches(quipThread.html, regexp);
        ...
        for(const reference of matchesReference) {
            wrappedHtml = await this._processReference(wrappedHtml, reference, pathDeepness);
        }
