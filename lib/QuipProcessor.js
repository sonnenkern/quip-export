...
class QuipProcessor {
    constructor (quipToken, saveCallback = ()=>{}, progressCallback = ()=>{}, phaseCallback = ()=>{}, fileExistsCallback = ()=>{},options={}) {
    ...
    async _processThreads(quipThreads, path) {
        for(const index in quipThreads) {
            if(this.options.skipExists) {
                if(this.fileExistsCallback(sanitizeFilename(`${quipThreads[index].thread.title.trim()}.html`), 'THREAD', path)){
                    this.threadsProcessed++;
                    console.log(`skip ${quipThreads[index].thread.title.trim()}\n`);
                    continue;
                }
            }
    ...
    async _processFolders(quipFolders, path) {
        ...
        //     promises.push(this._processFolder(quipFolders[indexs[i]], `${path}${sanitizeFilename(quipFolders[indexs[i]].folder.title)}/`));
        // }
        for(const index in quipFolders) {
            await this._processFolder(quipFolders[index], `${path}${sanitizeFilename(quipFolders[index].folder.title)}/`);
        }
    }
    ...
    async _processFolder(quipFolder, path) {
        ...
        if(threadIds.length > 0) {
            ...
            if(threads) {
                ...
                this.threadsProcessed++;
                console.log("Can't load the Child-Threads for Folder: " + path + '\n')
