...
class QuipProcessor {
    ...
    async _processReference(html, reference, pathDeepness) {
        ...
        if(referencedObject && referencedObject.folder) {
            if(referencedObject) {
                path = '../'.repeat(pathDeepness) + referencedObject.path + referencedObject.title;
            } else {
                this.logger.debug("_processReference: Couldn't find referencedObject for FolderId=" + reference.threadId);
            }
        } else {
            ...
            if(referencedObject) {
                path = '../'.repeat(pathDeepness) + referencedObject.path +
                    sanitizeFilename(referencedThread.thread.title.trim()) + '.html';
            } else {
