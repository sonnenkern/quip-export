    ...
    async _getThreadMessagesHtml(quipThread, path) {
        ...
        for(const message of messages) {
            ...
            for(const reference of matchesReferences) {
                ...
                if(referencedObject.user) {
                    text = text.replace(reference.match, `<span class="message--user">@${title}</span>`);
                } else {
                    //if(this.options.resolveReferences) {
            ...
            if(message.files) {
                for(const file of message.files) {
                    ...
                    if(Mime.getType(file.name).startsWith('image/')) {
                        ...
                        text += await this._processFile(imageHtml, fileMatch, path, true);
                    } else {
                        //file
            ...
            html += `<div class="message"><span class="message--user">${message.author_name}: </span>${text}</div>`;
        }
